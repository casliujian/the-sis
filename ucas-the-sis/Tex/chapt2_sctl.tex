...
在本小节，我们来详细介绍\sctlprov{}的证明搜索算法中的数据结构及伪代码。

首先，我们需要介绍\textit{公式模式}的概念：
...
\couic{First, let us define the notion of formula schema.
...
\sctlprov{}的证明搜索算法如图\ref{appendix:algorithm:main}所示。
...
\begin{figure}[h!]\footnotesize
	...
	\noindent\framebox{\parbox{.96\textwidth}{\hspace*{-0.3cm}
			\begin{algorithmic}[1]
				...
				\State let $\mathsf{c'= \textsf{cpt}^\emptyset(\vdash(s/x)\phi_1, \textsf{cpt}^\emptyset(\Gamma'\vdash k(s_1), c_1^{A_{11}}, \textsf{cpt}^\emptyset(...\textsf{cpt}^\emptyset({\Gamma}'\vdash k(s_n), c_1^{A_{1n}}, c_2^{A_{2n}})...)), c_2^{A_{20}})}$
				\State $\mathsf{c := c'}$
...
\begin{figure}[h!]\footnotesize
	...
	\noindent\framebox{\parbox{.96\textwidth}{\hspace*{-0.3cm}
			\begin{algorithmic}[1]
				...
				\State let $\mathsf{c'= \textsf{cpt}^{\emptyset}(\vdash(s/x)\phi_1, c_1^{A_{10}}, \textsf{cpt}^{\emptyset}(\Gamma'\vdash k(s_1), \textsf{cpt}^\emptyset(...\textsf{cpt}^\emptyset(\Gamma'\vdash k(s_n), c_1^{A_{1n}}, c_2^{A_{2n}})...), c_2^{A_{21}}))}$
				\State $\mathsf{c := c'}$
				\EndIf
