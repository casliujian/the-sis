...
\begin{figure}[h!]
	\small
	\centering
	\caption{\sctlm{}}\label{sctl_rules} 
...
\subsubsection{$\mathsf{ProveEG}$ 和 $\mathsf{ProveAF}$}
...
\end{figure}
	...
	\noindent\framebox{\parbox{.96\textwidth}{\hspace*{-0.3cm}
			\begin{algorithmic}[1]
				...
				\State \textit{``k(s)'' to $\mathit{EG_x(\phi_1)(s)}$.*/}
				\If{$\mathsf{s\in M^f_k}$}
				\State $\mathsf{c:=c_2^{A_2}}$
				
				\ElsIf{$\mathsf{s\in M^t_k}$ or $\mathsf{k(s)\in }\;\Gamma$}
				\State $\mathsf{c:=c_1^{A_1}}$
				\State $\mathsf{M^t_k := M^t_k \cup \mathsf{states}(}\Gamma\mathsf{)}$
				\Else 
				\State let $\mathsf{A_{20}=A_2\cup\{ac(ce,}\; \Gamma\;\mathsf{\vdash k(s),\{\vdash(s/x)\phi_1\})\}}$
				\State let $\mathsf{A_{2n}=A_2\cup\{ac(ce,}\;\Gamma\;\mathsf{\vdash k(s),\{}\Gamma'\;\mathsf{\vdash k(s_1),...,}\;\Gamma'\;\mathsf{\vdash k(s_n)\})\}}$
				\State $\mathsf{\forall i\in\{1,...,n\}}$, let 
				\State ~~~~$\mathsf{A_{1i}=A_1\cup\{ac(pt,}\; \Gamma\;\mathsf{\vdash k(s),\{\vdash(s/x)\phi_1,}\; \Gamma'\;\mathsf{\vdash k(s_i)\})\}}$
				\State $\mathsf{A_{20}:=A_{20} \cup \{M^f_k:= M^f_k\cup \{s\}\}}$;
				\State $\mathsf{A_{2n}:=A_{2n} \cup \{M^f_k:= M^f_k\cup \{s\}\}}$;
				\State let $\mathsf{c'= \textsf{cpt}^\emptyset(\vdash(s/x)\phi_1, \textsf{cpt}^\emptyset(}\Gamma'\;\mathsf{\vdash k(s_1), c_1^{A_{11}}, \textsf{cpt}^\emptyset(...\textsf{cpt}^\emptyset(}$
				\State $~~~~\Gamma'\;\mathsf{\vdash k(s_n), c_1^{A_{1n}}, c_2^{A_{2n}})...)), c_2^{A_{20}})}$
				\State $\mathsf{c := c'}$
...
\begin{figure}[h!]\footnotesize
	...
	\noindent\framebox{\parbox{.96\textwidth}{\hspace*{-0.3cm}
			\begin{algorithmic}[1]
				...
				\State \textit{``k(s)'' to $\mathit{AF_x(\phi_1)(s)}$.*/}
				\If{$\mathsf{s\in M^t_k}$}
				\State $\mathsf{c:=c_1^{A_1}}$
				
				\ElsIf{$\mathsf{s\in M^f_k}$ or $\mathsf{k(s)\in }\;\Gamma$}
				\State $\mathsf{c:=c_2^{A_2}}$
				\State $\mathsf{M^f_k := M^f_k \cup \mathsf{states}(}\Gamma\;\mathsf{)}$
				\Else 
				\State let $\mathsf{A_{10}=A_1\cup\{ac(pt,}\; \Gamma\;\mathsf{\vdash k(s),\{\vdash(s/x)\phi_1\})\})\}}$
				\State let $\mathsf{A_{1n}=A_1\cup\{ac(pt,}\; \Gamma\;\mathsf{\vdash k(s),\{}\Gamma'\;\mathsf{\vdash k(s_1),...,}\;\Gamma'\;\mathsf{\vdash k(s_n)\})\}}$
				\State $\mathsf{\forall i\in\{1,...,n\}}$, let
				\State~~~~ $\mathsf{A_{2i}=A_2\cup\{ac(ce,}\; \Gamma\;\mathsf{\vdash k(s),\{\vdash(s/x)\phi_1,}\; \Gamma'\;\mathsf{\vdash k(s_i)\})\}}$
				\State $\mathsf{A_{10}:=A_{10} \cup \{M^t_k:= M^t_k\cup \{s\}\}}$
				\State $\mathsf{A_{1n}:=A_{1n} \cup \{M^t_k:= M^t_k\cup  \{s\}\}}$;
				\State let $\mathsf{c'= \textsf{cpt}^{\emptyset}(\vdash(s/x)\phi_1, c_1^{A_{10}}, \textsf{cpt}^{\emptyset}(}\Gamma'\;\mathsf{\vdash k(s_1), \textsf{cpt}^\emptyset(...\textsf{cpt}^\emptyset(}$
				\State $~~~~\Gamma'\;\mathsf{\vdash k(s_n), c_1^{A_{1n}}, c_2^{A_{2n}})...), c_2^{A_{21}}))}$
				\State $\mathsf{c := c'}$
...
$\mathsf{ProveEG}$的伪代码细节如图\ref{algorithm:eg}所示，其中，$\mathsf{states(\Gamma)}$表示$\mathsf{\Gamma}$中出现的状态，并且令$\{s_1,...,s_n\}=\textsf{Next}(s)$，$\mathsf{\Gamma'=\Gamma,EG_x(\phi_1)(s)}$。
$\mathsf{ProveEG}$的解释如下：

\begin{itemize}
	\item 第3、4行： 如果已知$\mathsf{EG_x(\phi_1)(s)}$是不可证的，那么将$\mathsf{c}$重写到$\mathsf{c_2}$。
	\item 第5 -- 7行：如果已知$\mathsf{EG_x(\phi_1)(s)}$是可证的，或者可应用merge规则，那么将$\mathsf{c}$重写到$\mathsf{c_1}$，同时，由于${\Gamma}$中的每个公式都是可证的，因此将${\Gamma}$中出现的所有状态加入到$\mathsf{M_{EG_x(\phi_1)(\_)}^t}$中。
	\item 第15--16行： \textsf{CPT} $\mathsf{c'}$的构造方式如下：
	\begin{enumerate}
		\item 如果$\mathsf{c'}$重写到外层的$\mathsf{c_2}$，那么$\mathsf{\vdash(s/x)\phi_1}$是不可证的，因此将$\mathsf{\vdash(s/x)\phi_1}$作为$\Gamma\;\mathsf{\vdash EG_x(\phi_1)(s)}$的子节点加到反例中（第9行）。同时，如果 $\Gamma\;\mathsf{\vdash EG_x(\phi_1)(s)}$ 是不可证的，那么不存在以$s$开头的无穷路径使得该路径上的所有状态都满足$\phi_1$，在这种情况下，我们将$s$加入到 $\mathsf{M_{EG_x(\phi_1)(\_)}^f}$中（第13行）。
		
		\item 如果$\mathsf{c'}$重写到内层的$\mathsf{c_2}$，那么$\Gamma'\;\mathsf{\vdash EG_x(\phi_1)(s_1)}, ... , \Gamma'\;\mathsf{\vdash EG_x(\phi_1)(s_n)}$都是不可证的，因此将其都作为$\Gamma\;\mathsf{\vdash EG_x(\phi_1)(s)}$的子节点加入到反例中（第10行）。同时，$\Gamma\;\mathsf{\vdash EG_x(\phi_1)(s)}$是不可证的，因此将$s$加入到$\mathsf{M_{EG_x(\phi_1)(\_)}^f}$中（第14行）。
		\item 如果$\mathsf{c'}$重写到第$i$个$\mathsf{c_1}$，那么$\mathsf{\vdash(s/x)\phi_1}$和$\Gamma'\;\mathsf{\vdash EG_x(\phi_1)(s_i)}$都是可证的，因此将其都作为$\Gamma\;\mathsf{\vdash EG_x(\phi_1)(s)}$的子节点加入到证明树中（第11、12行）。 
	\end{enumerate}
	\item 第17行：将$\mathsf{c}$重写到$\mathsf{c'}$。
\end{itemize}
...
\begin{figure}[h!]\footnotesize
	...
	\caption{$\mathsf{ProveEU (}\Gamma\;\mathsf{\vdash EU_{x,y}(\phi_1,\phi_2)(s))}$}
	\label{algorithm:eu}
	\noindent\framebox{\parbox{.97\textwidth}{\hspace*{-0.3cm}
			\begin{algorithmic}[1]
				\State \textit{/* For notation purpose, here we refer ``k'' to} $\mathit{EU_{x,y}(\phi_1,\phi_2)(\_)}$ and
				\State \textit{``k(s)'' to $\mathit{EU_{x,y}(\phi_1,\phi_2)(s)}$.*/}
				\If{$\mathsf{s\in M^t_k}$}
				\State $\mathsf{c:=c_1^{A_1}}$
				\State $\mathsf{M_k^t := M_k^t\cup reachable(states(}\Gamma\mathsf{))}$
				\State $\mathsf{M_k^f := M_k^f\setminus reachable(states(}\Gamma\mathsf{))}$
				\ElsIf{$\mathsf{s\in M^f_k}$ or $\mathsf{k(s)\in }\;\Gamma$}
				\State $\mathsf{c:=c_2^{A_2}}$
				\State $\mathsf{M^f_k := M^f_k\cup states(}\Gamma\mathsf{)}$
				\Else 
				\State let $\mathsf{A_{10}=A_1\cup\{}$
				\State~~~~$\mathsf{ac(pt,} \;\Gamma\;\mathsf{\vdash k(s),\{\vdash(s/y)\phi_2\}),}$
				\State~~~~$\mathsf{M^t_k := M^t_k\cup \mathsf{reachable}(\mathsf{states}(}\Gamma\mathsf{)\cup \{s\}),}$
				\State ~~~~$\mathsf{M^f_k := M^f_k\setminus \mathsf{reachable}(\mathsf{states}(}\Gamma\mathsf{)\cup \{s\})\}}$
				\State let $\mathsf{A_{20}=A_2\cup\{}$
				\State ~~~~$\mathsf{ac(ce,}\; \Gamma\;\mathsf{\vdash k(s),\{\vdash(s/x)\phi_1, \vdash(s/y)\phi_2\}),}$
				\State ~~~~$\mathsf{M^f_k:= M^f_k\cup states(}\Gamma\mathsf{)\cup\{s\}\}}$
				\State let $\mathsf{A_{2n}=A_2\cup\{\mathsf{ac}(ce,}\; \Gamma\;\mathsf{\vdash k(s),\{}\Gamma'\;\mathsf{\vdash k(s_1),...,}\;\Gamma'\;\mathsf{\vdash k(s_n)\})\}}$
				\State $\mathsf{\forall i\in\{1,...,n\}}$ , let 
				\State ~~~~$\mathsf{A_{1i}=A_1\cup\{ac(pt,}\; \Gamma\;\mathsf{\vdash k(s),\{\vdash(s/x)\phi_1,}\; \Gamma'\;\mathsf{\vdash k(s_i)\})\}}$
				\If{$\Gamma \;\mathsf{= \emptyset}$}	
...
\begin{figure}[h!]\footnotesize
	...
	\noindent\framebox{\parbox{.97\textwidth}{\hspace*{-0.3cm}
			\begin{algorithmic}[1]
				...
				\State \textit{``k(s)'' to $\mathit{AR_{x,y}(\phi_1,\phi_2)(s)}$.*/}
				\If{$\mathsf{s\in M^f_k}$}
				\State $\mathsf{c:=c_2^{A_2}}$
				\State $\mathsf{M_k^f := M_k^f\cup reachable(states(}\Gamma\mathsf{))}$
				\State $\mathsf{M_k^t := M_k^t\setminus reachable(states(}\Gamma\mathsf{))}$
				\ElsIf{$\mathsf{s\in M^t_k}$ or $\mathsf{k(s)\in}\; \Gamma$}
				\State $\mathsf{c:=c_1^{A_1}}$
				\State $\mathsf{M_k^t := M_k^t\cup states(}\Gamma\mathsf{)}$
				\Else 
				\State let $\mathsf{A_{10}=A_1\cup\{}$
				\State ~~~~$\mathsf{ac(pt,}\; \Gamma\;\mathsf{\vdash k(s),\{\vdash(s/x)\phi_1,\vdash(s/y)\phi_2\}),}$
				\State ~~~~$\mathsf{M_k^t := M_k^t\cup states(}\Gamma\mathsf{)\cup\{s\}\}}$
				\State let $\mathsf{A_{1n}=A_1\cup\{\mathsf{ac}(pt,}\; \Gamma\;\mathsf{\vdash k(s),\{}\Gamma'\;\mathsf{\vdash k(s_1),...,}\;\Gamma'\;\mathsf{\vdash k(s_n)\})\}}$
				\State let $\mathsf{A_{20}=A_2\cup\{}$
				\State~~~~$\mathsf{ac(ce,}\; \Gamma\;\mathsf{\vdash k(s),\{\vdash(s/y)\phi_2\}),}$
				\State ~~~~$\mathsf{M^f_k := M^f_k\cup \mathsf{reachable}(\mathsf{states}(}\Gamma\mathsf{)\cup \{s\}),}$
				\State ~~~~$\mathsf{M^t_k := M^t_k\setminus \mathsf{reachable}(\mathsf{states}(}\Gamma\mathsf{)\cup\{s\})}$
				\State $\mathsf{\forall i\in\{1,...,n\}}$, let 
				\State ~~~~$\mathsf{A_{2i}=A_2\cup\{\mathsf{ac}(ce,}\; \Gamma\;\mathsf{\vdash k(s),\{\vdash (s/x)\phi_1,}\; \Gamma'\;\mathsf{\vdash k(s_i)\})\}}$
				\If{$\Gamma\;\mathsf{ = \emptyset}$}
				\State $\mathsf{\mathsf{visited}_k := \{s\}}$
...
\end{figure}
$\mathsf{ProveEU}$的伪代码细节要比$\mathsf{ProveEG}$更加复杂。原因是，对于后者我们只需找到一个满足某个公式的环即可，而对于前者我们需要找到一条终点状态满足某公式的有穷路径，而且在找到这条路径之前可能已访问若干个环。$\mathsf{ProveEU}$的伪代码细节如图\ref{algorithm:eu}所示。
在$\mathsf{ProveEU}$中，我们令$\{s_1,...,s_n\}=\textsf{Next}(s)$；
令 $\mathsf{reachable(S)}$ 表示能经过有穷路径到达$\mathsf{S}$的某个状态的状态集合，其中$\mathsf{S}$表示某个有穷路径上状态的集合。$\mathsf{ProveEU}$的解释如下：

\begin{itemize}
	\item 第3 -- 6行： 如果$\mathsf{EU_{x,y}(\phi_1,\phi_2)(s)}$是可证的，那么将$\mathsf{c}$重写到$\mathsf{c_1}$。同时，将$\mathsf{reachable(states(}\Gamma\mathsf{))}$中的所有状态加入到集合$\mathsf{M_{EU_{x,y}(\phi_1,\phi_2)(\_)}^t}$中，并从集合$\mathsf{M_{EU_{x,y}(\phi_1,\phi_2)(\_)}^f}$中移除。 
	\item 第7 -- 9行：如果状态$\mathsf{s}$已被访问，那么将$\mathsf{c}$ 重写到 $\mathsf{c_2}$。同时，将集合$\mathsf{states(}\Gamma\mathsf{)}$中的所有的状态加入到集合$\mathsf{M_{EU_{x,y}(\phi_1,\phi_2)(\_)}^f}$中以避免重复访问。
	\item 第26、27行：\textsf{CPT} $\mathsf{c'}$的构造方式如下：
	\begin{enumerate}
		\item 如果$\mathsf{c'}$将来能重写到第一个$\mathsf{c_1}$，那么$\mathsf{\vdash(s/y)\phi_2}$是可证的，因此将其作为$\Gamma\;\mathsf{\vdash EU_{x,y}(\phi_1,\phi_2)(s)}$的子节点加到证明树中（第12行）。与此同时，如果$\mathsf{(s/y)\phi_2}$是可证的，那么对于任意访问过的状态$s'$，如果存在以$s'$为起点的有穷路径而$s$在这条路径上，则$\mathsf{EU_{x,y}(\phi_1,\phi_2)(s')}$是可证的。因此，可将状态集合$\mathsf{reachable(states(}\Gamma\mathsf{)\cup\{s\})}$中的所有状态加入到 $\mathsf{M_{EU_{x,y}(\phi_1,\phi_2)(\_)}^t}$中并从集合$\mathsf{M_{EU_{x,y}(\phi_1,\phi_2)(\_)}^f}$移除（第13、14行）。
		\item 如果$\mathsf{c'}$能重写到第$i$个其他的$\mathsf{c_1}$，那么$\vdash(s/x)\phi_1$和 $\Gamma'\;\mathsf{\vdash EU_{x,y}(\phi_1,\phi_2)(s_i)}$均是可证的，因此将其都作为$\Gamma\;\mathsf{\vdash EU_{x,y}(\phi_1,\phi_2)(s)}$的子节点加入到证明树中（第19、20行）。
		
		\item 如果$\mathsf{c'}$重写到外层的$\mathsf{c_2}$，那么$\mathsf{\vdash (s/x)\phi_1}$和$\mathsf{\vdash (s/y)\phi_2}$是不可证的，因此将其都作为$\Gamma\;\mathsf{\vdash EU_{x,y}(\phi_1,\phi_2)(s)}$的子节点加入到反例中（第16行）。同时，将集合$\mathsf{states(}\Gamma\mathsf{)\cup \{s\}}$中的所有状态加入到$\mathsf{M_{EU_{x,y}(\phi_1,\phi_2)(\_)}^f}$中以防止重复访问（第17行）。
		\item 如果$\mathsf{c'}$重写到内层的$\mathsf{c_2}$，那么$\Gamma'\;\mathsf{\vdash EU_{x,y}(\phi_1,\phi_2)(s_1)},...,\;\Gamma'\;\mathsf{\vdash EU_{x,y}(\phi_1,\phi_2)(s_n)}$ 都是不可证的，因此将其全部作为$\Gamma\;\mathsf{\vdash EU_{x,y}(\phi_1,\phi_2)(s)}$的子节点加入到反例中（第18行）。
	\end{enumerate}
	\item 第28行：将$\mathsf{c}$重写到$\mathsf{c'}$。
\end{itemize}
