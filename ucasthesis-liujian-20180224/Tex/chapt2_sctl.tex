...
\begin{proof}
	...
	for a given formula $\phi$, \couic{ where $c_1$ and $c_2$ cannot rewrite to each other,} $\textsf{cpt}(\Gamma \vdash\phi, c_1, c_2) 
...
In the proof search of sequents with co-inductive formulae (formulae with modality $EG$ or
$AR$), the merge rules are used to assert that some property holds on
an infinite path of states. For every merge rule, the formulae need to be
memoized are with the same modality, whereas the only differences are
the states appearing in the formulae. Thus, it is sufficient to
memoize only the states, not the whole formulae, in the
implementation of every merge (i.e., $\Gamma$). Essentially, each
construction of a merge is implemented by memoizing an infinite path
where all states verify some property. 

What is worth mentioning is the proof search of sequents with
...
\begin{figure}[h]
	\footnotesize
	\noindent\framebox{\parbox{.46\textwidth}{\hspace*{-0.3cm}
...
In the proof search algorithm, the rewriting steps for the cases where $\phi$ is an atomic formula or the negation of a atomic formula is presented, and the explanation for other cases are presented separately in the following Subsections.
...
\begin{figure}[h!]\footnotesize
	\noindent\framebox{\parbox{.46\textwidth}{\hspace*{-0.3cm}
			\begin{algorithmic}[1]
				\State \textit{/* For notation purpose, here we refer ``k'' to} $\mathit{EX_x(\phi_1)(\_)}$, and
				\State \textit{``k(s)'' to $\mathit{EX_x(\phi_1)(s)}$.*/}
				...
				\State let $\mathsf{c'= \textsf{cpt}^\emptyset(\vdash(s_1/x)\phi_1, c_1^{A_{11}}, \textsf{cpt}^\emptyset(...\textsf{cpt}^\emptyset(\vdash(s_n/x)\phi_1, c_1^{A_{1n}}, }$
				\State ~~~~$\mathsf{c_2^{A_2})...))}$
				\State $\mathsf{c := c'}$
...
Figure \ref{algorithm:ax} shows the details of $\mathsf{ProveAX}$, which is the dual case of $\mathsf{ProveEX}$.
\begin{figure}[h!]\footnotesize
	\noindent\framebox{\parbox{.46\textwidth}{\hspace*{-0.3cm}
			\begin{algorithmic}[1]
				\State \textit{/* For notation purpose, here we refer ``k'' to} $\mathit{AX_x(\phi_1)(\_)}$, and
				\State \textit{``k(s)'' to $\mathit{AX_x(\phi_1)(s)}$.*/}
				...
				\State let $\mathsf{c'= \textsf{cpt}^\emptyset(\vdash(s_1/x)\phi, \textsf{cpt}^\emptyset(...\textsf{cpt}^\emptyset(\vdash(s_n/x)\phi, c_1^{A_{1}}, c_2^{A_{2n}}),...), }$
				\State~~~~$\mathsf{c_2^{A_{21}})}$
				\State $\mathsf{c := c'}$
...
\begin{itemize}
	...
	\begin{enumerate}
		...
		\couic{If, at the same time, $\Gamma$ is  $\emptyset$, then all visited states should be added into $\mathsf{M_{EG_x(\phi_1)(\_)}^f}$ since $\mathsf{\textsf{cpt}(\vdash EG_x(\phi_1)(s), c_1, c_2)\rsa^*c_2}$ indicates that all visited $EG$ formulae during the rewriting are not provable (Line 16 -- 17).}
...
\begin{figure}[h!]\footnotesize
	\noindent\framebox{\parbox{.47\textwidth}{\hspace*{-0.3cm}
			\begin{algorithmic}[1]
				\State \textit{/* For notation purpose, here we refer ``k'' to} $\mathit{EG_x(\phi_1)(\_)}$ and
				\State \textit{``k(s)'' to $\mathit{EG_x(\phi_1)(s)}$, and let $\Gamma'$ be $\Gamma\cup\{k(s)\}$.*/}
				...
				\Else 
				\State let $\mathsf{A_{20}=A_2\cup\{\mathsf{ac}(ce, \Gamma\vdash k(s),\{\vdash(s/x)\phi_1\})\}}$
				\State let $\mathsf{A_{2n}=A_2\cup\{\mathsf{ac}(ce, \Gamma\vdash k(s),\{\Gamma'\vdash k(s_1),...,\Gamma'\vdash k(s_n)\})\}}$
				\State $\mathsf{\forall i\in\{1,...,n\}}$, let 
				\State ~~~~$\mathsf{A_{1i}=A_1\cup\{\mathsf{ac}(pt, \Gamma\vdash k(s),\{\vdash(s/x)\phi_1, \Gamma\vdash k(s_i)\})\}}$
				...
				\State let $\mathsf{c'= \textsf{cpt}^\emptyset(\vdash(s/x)\phi_1, \textsf{cpt}^\emptyset(\Gamma'\vdash k(s_1), c_1^{A_{11}}, \textsf{cpt}^\emptyset(...\textsf{cpt}^\emptyset(}$
				\State $~~~~\mathsf{\Gamma'\vdash k(s_n), c_1^{A_{1n}}, c_2^{A_{2n}})...)), c_2^{A_{20}})}$
				\State $\mathsf{c := c'}$
	...
	\label{algorithm:eg}
\end{figure}
Figure \ref{algorithm:af} shows the details of $\mathsf{ProveAF}$, which is the dual case of $\mathsf{ProveEG}$.
\begin{figure}[h!]\footnotesize
	\noindent\framebox{\parbox{.47\textwidth}{\hspace*{-0.3cm}
			\begin{algorithmic}[1]
				\State \textit{/* For notation purpose, here we refer ``k'' to} $\mathit{AF_x(\phi_1)(\_)}$ and
				\State \textit{``k(s)'' to $\mathit{AF_x(\phi_1)(s)}$, and let $\Gamma'$ be $\Gamma\cup\{k(s)\}$..*/}
				...
				\Else 
				\State let $\mathsf{A_{10}=A_1\cup\{\mathsf{ac}(pt, \vdash k(s),\{\vdash(s/x)\phi_1\})\})\}}$
				\State let $\mathsf{A_{1n}=A_1\cup\{\mathsf{ac}(pt, \vdash k(s),\{\vdash k(s_1),...,\vdash k(s_n)\})\}}$
				\State $\mathsf{\forall i\in\{1,...,n\}}$, let
				\State~~~~ $\mathsf{A_{2i}=A_2\cup\{\mathsf{ac}(ce, \vdash k(s),\{\vdash(s/x)\phi_1, \vdash k(s_i)\})\}}$
				%				\State~~~~~~~~$\mathsf{M^f_k := M^f_k\cup \mathsf{reachable}(\mathsf{states}(\Gamma')),}$
				%				\State~~~~~~~~$\mathsf{M^t_k := M^t_k\cup \mathsf{visited}_k\setminus \mathsf{reachable}(\mathsf{states}(\Gamma'))\}}$
				\If{$\mathsf{\Gamma = \emptyset}$}
				\State $\mathsf{\mathsf{visited}_k := \{s\}}$
				\Else
				\State $\mathsf{\mathsf{visited}_k := \mathsf{visited}_k\cup\{s\}}$
				...
				\State let $\mathsf{c'= \textsf{cpt}^{\emptyset}(\vdash(s/x)\phi_1, c_1^{A_{10}}, \textsf{cpt}^{\emptyset}(\Gamma'\vdash k(s_1), \textsf{cpt}^\emptyset(...\textsf{cpt}^\emptyset(}$
				\State $~~~~\mathsf{\Gamma'\vdash k(s_n), c_1^{A_{1n}}, c_2^{A_{2n}})...), c_2^{A_{21}}))}$
				\State $\mathsf{c := c'}$
	...
	\caption{$\mathsf{ProveAF (\Gamma\vdash AF_x(\phi_1)(s))}$}
	\label{algorithm:af}
\end{figure}

\subsubsection{$\mathsf{ProveEU}$ and $\mathsf{ProveAR}$}
...
\couic{The algorithm for $\mathsf{ProveEU}$ is explained as follows.}
\begin{itemize}
	...
	\begin{enumerate}
		...
		\item If $\mathsf{c'}$ rewrites to the outer $\mathsf{c_2}$, then both $\mathsf{\vdash (s/x)\phi_1}$ and $\mathsf{\vdash (s/y)\phi_2}$ should be added to the counterexample as the children of $\mathsf{\Gamma\vdash EU_{x,y}(\phi_1,\phi_2)(s)}$ (Line 12).
		\item If $\mathsf{c'}$ rewrites to the inner $\mathsf{c_2}$, then $\mathsf{\Gamma'\vdash EU_{x,y}(\phi_1,\phi_2)(s_1)},...,\mathsf{\Gamma'\vdash EU_{x,y}(\phi_1,\phi_2)(s_n)}$ should all be added to the counterexample as the children of $\mathsf{\Gamma\vdash EU_{x,y}(\phi_1,\phi_2)(s)}$ (Line 13).
		\item If $\mathsf{\Gamma\vdash EU_{x,y}(\phi_1,\phi_2)(s)}$ is not provable, then add $s$ to $\mathsf{M_{EU_{x,y}(\phi_1,\phi_2)(\_)}^f}$ in order to avoid visiting $\mathsf{s}$ repeatedly in the following steps (Line 24 -- 25). Note that $\mathsf{s}$ may be removed from $\mathsf{M_{EU_{x,y}(\phi_1,\phi_2)(\_)}^f}$ (Line 11 and Line 18). This occurs only when $\mathsf{\Gamma'\vdash EU_{x,y}(\phi_1,\phi_2)(s')}$ is provable for some $\mathsf{\Gamma'}$ and $\mathsf{s'}$, $\mathsf{\vdash (s'/y)\phi_2}$ is provable, and $\mathsf{s\in reachable(states(\Gamma')\cup \{s'\})}$. 
	...
	\end{enumerate}
	\item Line 28: Rewrite $\mathsf{c}$ to $\mathsf{c'}$. 
\end{itemize}

Figure \ref{algorithm:ar} shows the details of $\mathsf{ProveAR}$, which is the dual case of $\mathsf{ProveEU}$.

\begin{figure}[h!]\footnotesize
	\noindent\framebox{\parbox{.48\textwidth}{\hspace*{-0.3cm}
			\begin{algorithmic}[1]
				\State \textit{/* For notation purpose, here we refer ``k'' to} $\mathit{EU_{x,y}(\phi_1,\phi_2)(\_)}$ and
				\State \textit{``k(s)'' to $\mathit{EU_{x,y}(\phi_1,\phi_2)(s)}$, and let $\Gamma'$ be $\Gamma\cup\{k(s)\}$.*/}
				\If{$\mathsf{s\in M^t_k}$}
				\State $\mathsf{c:=c_1^{A_1}}$
				%				\State $\mathsf{M^t_k := M^t_k\cup \mathsf{reachable}(\mathsf{states}(\Gamma))}$
				%				\State $\mathsf{M^f_k := M^f_k\cup \mathsf{visited}_k\setminus \mathsf{reachable}(\mathsf{states}(\Gamma))}$
				...
				\State~~~~$\mathsf{M^t_k := M^t_k\cup \mathsf{reachable}(\mathsf{states}(\Gamma)\cup \{s\}),}$
				\State ~~~~$\mathsf{M^f_k := (M^f_k\cup \mathsf{visited}_k)\setminus \mathsf{reachable}(\mathsf{states}(\Gamma)\cup \{s\})}$
				\State let $\mathsf{A_{20}=A_2\cup\{\mathsf{ac}(ce, \Gamma\vdash k(s),\{\vdash(s/x)\phi_1, \vdash(s/y)\phi_2\})\}}$
				\State let $\mathsf{A_{2n}=A_2\cup\{\mathsf{ac}(ce, \Gamma\vdash k(s),\{\Gamma'\vdash k(s_1),...,\Gamma'\vdash k(s_n)\})\}}$
				\State $\mathsf{\forall i\in\{1,...,n\}}$ , let 
				\State ~~~~$\mathsf{A_{1i}=A_1\cup\{}$
				\State~~~~~~~~$\mathsf{ac(pt, \Gamma\vdash k(s),\{\Gamma'\vdash k(s_i)\}),}$
				\State~~~~~~~~$\mathsf{M^t_k := M^t_k\cup \mathsf{reachable}(\mathsf{states}(\Gamma')\cup \{s_i\}),}$
				\State~~~~~~~~$\mathsf{M^f_k := (M^f_k\cup \mathsf{visited}_k)\setminus \mathsf{reachable}(\mathsf{states}(\Gamma')\cup \{s_i\})\}}$
				%					\State~~~~~~~~$\mathsf{\}}$
				%				\State $\mathsf{A_{10} := A_{10}\cup \{}$
				%				\State~~~~$\mathsf{M^t_k := M^t_k\cup \mathsf{reachable}(\mathsf{states}(\Gamma)),}$
				%				\State ~~~~$\mathsf{M^f_k := M^f_k\cup \mathsf{visited}_k\setminus \mathsf{reachable}(\mathsf{states}(\Gamma))\}}$
				\If{$\mathsf{\Gamma = \emptyset}$}
				
				\State $\mathsf{visited}_k := \{s\}$
				\Else
				\State $\mathsf{\mathsf{visited}_k := \mathsf{visited}_k\cup\{s\}}$
				\EndIf
				\State $\mathsf{A_{20}:=A_{20} \cup \{M^f_k:= M^f_k\cup \{s\}\}}$
				\State $\mathsf{A_{2n}:=A_{2n} \cup \{M^f_k:= M^f_k\cup \{s\}\}}$
				\State let $\mathsf{c'= \textsf{cpt}^\emptyset(\vdash(s/y)\phi_2, c_1^{A_{10}}, \textsf{cpt}^\emptyset(\vdash(s/x)\phi_1,\textsf{cpt}^\emptyset(\Gamma'\vdash k(s_1), }$
				\State $~~~~\mathsf{c_1^{A_{11}}, \textsf{cpt}^\emptyset(...\textsf{cpt}^\emptyset(\Gamma'\vdash k(s_n), c_1^{A_{1n}}, c_2^{A_{2n}})...)), c_2^{A_{20}}))}$
				\State $\mathsf{c := c'}$
				\EndIf
			\end{algorithmic}
	}}
	\caption{$\mathsf{ProveEU (\Gamma\vdash EU_{x,y}(\phi_1,\phi_2)(s))}$}
	\label{algorithm:eu}
\end{figure}
...
\begin{figure}[h!]\footnotesize
	\noindent\framebox{\parbox{.48\textwidth}{\hspace*{-0.3cm}
			\begin{algorithmic}[1]
				\State \textit{/* For notation purpose, here we refer ``k'' to} $\mathit{AR_{x,y}(\phi_1,\phi_2)(\_)}$ and
				\State \textit{``k(s)'' to $\mathit{AR_{x,y}(\phi_1,\phi_2)(s)}$, and let $\Gamma'$ be $\Gamma\cup\{k(s)\}$.*/}
				\If{$\mathsf{s\in M^f_k}$}
				\State $\mathsf{c:=c_2^{A_2}}$
				%				\State $\mathsf{M^f_k := M^f_k\cup \mathsf{reachable}(\mathsf{states}(\Gamma))}$
				%				\State $\mathsf{M^t_k := M^t_k\cup \mathsf{visited}_k\setminus \mathsf{reachable}(\mathsf{states}(\Gamma))}$
				...
				\State $\mathsf{c:=c_1^{A_1}}$;
				\Else 
				\State let $\mathsf{A_{10}=A_1\cup\{\mathsf{ac}(pt, \Gamma\vdash k(s),\{\vdash(s/x)\phi_1,\vdash(s/y)\phi_2\})\}}$
				\State let $\mathsf{A_{1n}=A_1\cup\{\mathsf{ac}(pt, \Gamma\vdash k(s),\{\Gamma'\vdash k(s_1),...,\Gamma'\vdash k(s_n)\})\}}$
				\State let $\mathsf{A_{20}=A_2\cup\{}$
				\State~~~~$\mathsf{ac(ce, \Gamma\vdash k(s),\{\vdash(s/y)\phi_2\}),}$
				\State ~~~~$\mathsf{M^f_k := M^f_k\cup \mathsf{reachable}(\mathsf{states}(\Gamma)\cup \{s\}),}$
				\State ~~~~$\mathsf{M^t_k := (M^t_k\cup \mathsf{visited}_k)\setminus \mathsf{reachable}(\mathsf{states}(\Gamma)\cup\{s\})}$
				\State $\mathsf{\forall i\in\{1,...,n\}}$, let 
				\State ~~~~$\mathsf{A_{2i}=A_2\cup\{}$
				\State~~~~~~~~$\mathsf{\mathsf{ac}(ce, \Gamma\vdash k(s),\{\Gamma'\vdash k(s_i)\}),}$
				\State~~~~~~~~$\mathsf{M^f_k := M^f_k\cup \mathsf{reachable}(\mathsf{states}(\Gamma')\cup\{s_i\}),}$
				\State~~~~~~~~$\mathsf{M^t_k := M^t_k\cup \mathsf{visited}_k\setminus \mathsf{reachable}(\mathsf{states}(\Gamma')\cup\{s_i\})\}}$
				%				\State $\mathsf{A_{20} := A_{20}\cup \{}$
				%				\State ~~~~$\mathsf{M^f_k := M^f_k\cup \mathsf{reachable}(\mathsf{states}(\Gamma)),}$
				%				\State ~~~~$\mathsf{M^t_k := M^t_k\cup \mathsf{visited}_k\setminus \mathsf{reachable}(\mathsf{states}(\Gamma))\}}$
				\If{$\mathsf{\Gamma = \emptyset}$}
				\State $\mathsf{\mathsf{visited}_k := \{s\}}$
				...
				\State $\mathsf{A_{10}:=A_{10} \cup \{M^t_k:= M^t_k\cup \{s\}\}}$
				\State $\mathsf{A_{1n}:=A_{1n} \cup \{M^t_k:= M^t_k\cup \{s\}\}}$
				\State let $\mathsf{c'= \textsf{cpt}^\emptyset(\vdash(s/y)\phi_2, \textsf{cpt}^\emptyset(\vdash (s/x)\phi_1, c_1^{A_{10}}, \textsf{cpt}^\emptyset(\Gamma'\vdash k(s_1), }$
				\State ~~~~$\mathsf{\textsf{cpt}^\emptyset(...\textsf{cpt}^\emptyset(\Gamma'\vdash k(s_n), c_1^{A_{1n}}, c_2^{A_{2n}})...), c_2^{A_{21}})), c_2^{A_{20}})}$
				\State $\mathsf{c := c'}$
	...
	\caption{$\mathsf{ProveAR (\Gamma\vdash AR_{x,y}(\phi_1,\phi_2)(s))}$}
	\label{algorithm:ar}
\end{figure}
%}
